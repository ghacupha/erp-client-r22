...
describe('PurchaseOrder e2e test', () => {
  ...
  /* it('should create and delete PurchaseOrders', async () => {
        const beforeRecordsCount = await isVisible(purchaseOrderComponentsPage.noRecords) ? 0 : await getRecordsCount(purchaseOrderComponentsPage.table);
        purchaseOrderUpdatePage = await purchaseOrderComponentsPage.goToCreatePurchaseOrder();
        await purchaseOrderUpdatePage.enterData();
        expect(await isVisible(purchaseOrderUpdatePage.saveButton)).to.be.false;
