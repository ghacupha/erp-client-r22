...
export default class SubCountyCodeUpdatePage {
  pageTitle: ElementFinder = element(by.id('erpSystemR22App.subCountyCode.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  countyCodeInput: ElementFinder = element(by.css('input#sub-county-code-countyCode'));
  countyNameInput: ElementFinder = element(by.css('input#sub-county-code-countyName'));
  subCountyCodeInput: ElementFinder = element(by.css('input#sub-county-code-subCountyCode'));
  subCountyNameInput: ElementFinder = element(by.css('input#sub-county-code-subCountyName'));
  placeholderSelect: ElementFinder = element(by.css('select#sub-county-code-placeholder'));
  ...
  async enterData() {
    ...
    await this.setCountyCodeInput('countyCode');
    await waitUntilDisplayed(this.saveButton);
    await this.setCountyNameInput('countyName');
    await waitUntilDisplayed(this.saveButton);
    await this.setSubCountyCodeInput('subCountyCode');
    await waitUntilDisplayed(this.saveButton);
